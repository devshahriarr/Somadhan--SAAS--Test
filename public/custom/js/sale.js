...
// const customerId1 = document.querySelector('.select-customer').value;
...
$(document).ready(function () {
                ...
                if (products.length > 0) {
                    ...
                    $.each(products, function (index, product) {
                        ...
                        const b2c_price = variations.b2c_price
                            ...
                            : '  N/A';
                        $(".view_product").append(
                            `<option value="${product.id}">${product.name} (${product.stock_quantity_sum || 0
                            } pc Available)
                            | ${['admin', 'superadmin'].includes(userRole) ? ` | Cost Price: <i>${costPrice}</i>` : ''
                            }${['admin', 'superadmin', 'salesman'].includes(userRole) ? ` | B2B Price: <i>${b2b_price}</i>` : ''
                            }${['admin', 'superadmin', 'salesman'].includes(userRole) ? ` | B2C Price: <i>${b2c_price}</i>` : ''
                            }|
                            </option>`
                        );
                    });
    ...
    function showAddProduct(product, quantity, saleItemsPrice, promotion) {
        ...
        const priceOptions =
            Array.isArray(saleItemsPrice) && saleItemsPrice.length > 0
                ? saleItemsPrice
                    .map(
                        (price) =>
        ...
        // Determine if we should use the select or fallback price //
        const useFallbackPrice =
            !Array.isArray(saleItemsPrice) || saleItemsPrice.length === 0;
        const fallbackPrice =
            priceType == "b2b_price" ? product.b2b_price : product.b2c_price;

        // console.log(priceOptions);//
        // If the row doesn't exist, add a new row

        const rowHtml = `
            <tr class="data_row${product.id}"  id="fa-bars">
              <td>${serialNumber++}</td>
            <td class="fa-barss">
            ...
            " style="width:100%; margin: 0; padding: 0; font-size: 12px; white-space: normal; word-wrap: break-word; max-height: 50px; overflow-y: auto;">${product?.product?.name ?? ""
                ...
                <input type="text"
                class="form-control product_color${product.id
            } border-0" name="product_color[]" readonly value="${product?.color_name?.name ?? ""
            }" style="width:60px;margin:0;padding:0;font-size:12px"   />
                </td>
                <td  ${size_view === "0" ? "hidden" : ""}>
                    <input
                      type="text" class="form-control product_size${product.id
            } border-0" name="product_size[]" readonly value="${product?.variation_size?.size ?? ""
            }"  style="width:60px;margin:0;padding:0;font-size:12px"  />
                </td>
                <td>
                    <input type="hidden" class="product_id" name="product_id[]" readonly value="${product.id ?? 0
            }" />
                    <input type="hidden" class="variant_cost_price${product.id
            }" name="variant_cost_price[]" readonly value="${product.cost_price ?? 0
            }" />
                    <input type="hidden" class="variant_sale_price${product.id
            }" name="variant_sale_price[]" readonly value="${priceType === "b2b_price"
                ? product.b2b_price
                : product.b2c_price ?? 0
            }"  />

                    <input type="hidden" class="promotion_type${product.id
            }" name="promotion_type[]" readonly value="${promotion?.discount_type
            }" />
                    <input type="hidden" class="discount_value${product.id
            }" name="discount_value[]" readonly value="${promotion?.discount_value
            }" />
                    <input type="number" product-id="${product.id
            }" class="form-control unit_price product_price${product.id
            } ${checkSellEdit == 0 ? "border-0" : ""
            }" id="product_price" name="unit_price[]" ${checkSellEdit == 0 ? "readonly" : ""
            } value="${party_ways_rate_kit == 1 && !useFallbackPrice
                ? saleItemsPrice[0]?.rate
                : priceType == "b2b_price"
                    ? product.b2b_price
                    : product.b2c_price
            }" style="width:60px;margin:0;padding:3px;font-size:12px"   />
          ${party_ways_rate_kit == 1 && !useFallbackPrice
                ? `
                <select class="form-control unit_price variant_sale_price${product.id}" name="variant_sale_price[]" style="width:60px;margin:0;padding:3px;font-size:12px">
            ...
            `
                ...
                <td>
                    <input type="number" product-id="${product.id
            }" class="form-control quantity productQuantity${product.id
            }" name="quantity[]" style="width:60px;margin:0;padding:3px;font-size:12px"  value="${quantity1}" />
                </td>
        ...
        $(`.variant_sale_price${product.id}`).on("change", function () {
            const selectedPrice = $(this).val();
            $(`.product_price${product.id}`).val(selectedPrice);
        });
    ...
    function generateSubtotalSection(product, promotion, quantity) {
        if (!promotion) {
            ...
            return `<input type="number" class="form-control product_subtotal${product.id
                } border-0" name="total_price[]" id="productTotal" readonly value="${price.toFixed(2) ?? 0
                }" style="width:60px;margin:0;padding:0;font-size:12px"  />`;
        }
            ...
            $(".show_variants").append(`
                <div class="col-md-4 p-2 text-center addVariant cursor-pointer" data-active="false" data-id="${variant.id
                    ...
                    <div class="border rounded overflow-hidden pb-2">
                        ...
                        <div class="mt-2">
                            <p class="">${priceType === "b2b_price"
                    ? variant.b2b_price
                    : variant.b2c_price ?? 0
                } <span> (${totalStock} pc )</span></p>
                            <p class="">${variant?.variation_size?.size ?? ""
                }</p>
    ...
    function viewCustomerDetails(customer) {
        $(".show_customer_details").html(`
            <div class="col-lg-12 d-flex align-items-center justify-content-between">
                ...
                <span>:</span>
                <label for="exampleInputUsername2" class="col-form-label">${customer.name ?? ""
            }</label>
            </div>
            <div class="col-lg-12 d-flex align-items-center justify-content-between">
                <label for="exampleInputUsername2" class="col-form-label">Customer Phone</label>
                <span>:</span>
                <label for="exampleInputUsername2" class="col-form-label">${customer?.phone ?? ""
            }</label>
            </div>
            <div class="col-lg-12 d-flex align-items-center justify-content-between">
                <label for="exampleInputUsername2" class="col-form-label">Previous Due</label>
                <span>:</span>
                <label for="exampleInputUsername2" class="col-form-label">${customer?.wallet_balance ?? "00"
            }</label>
            </div>
         `);
    }
        ...
        $('tr[class^="data_row"]').each(function () {
            ...
            if (sale_with_low_price == 0) {
                if (unit_price < variant_cost_price) {
                    toastr.warning(
                        `Cannot sell at a lower price than the cost price. Error in row .`
                    );
