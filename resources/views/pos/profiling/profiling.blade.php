        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                  ...
                  <input type="checkbox" class="row-checkbox-unused"
                   ...
                   data-due="${unusePayment.balance}">
                    </td>
                    <td>${unusePayment.date ?? ''}</td>
                    <td>${unusePayment.payment_type ?? ''}</td>
                    <td>${unusePayment?.balance ?? 'N/A'}</td>
            ...
            function updateTotalDue() {
             ...
             function unusedTotalAmount() {
                ...
                //  console.log('Total Due Sum:', totalUnusedSum);
                // $('#payment_balance').val(totalDueSum);
                 $('#unused_balance').html('Selected Unused Amount: <span style="color: green;">' + totalUnusedSum + ' ৳</span>');
            ...
            addlLnk_payment.addEventListener('click', function(e) {
                ...
                if (paymentAmount > totalDue) {
                    toastr.error(
                        `Payment amount (${paymentAmount}) cannot exceed total due (${totalDue.toFixed(2)})`
                        );
                    return;
                }
