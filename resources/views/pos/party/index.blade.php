@extends('master')
@section('title', '| Party Page')
@section('admin')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
            <li class="breadcrumb-item active" aria-current="page">Party Manage</li>
        </ol>
    </nav>
    <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="card-title">Party Table</h6>
                    ...
                    <div><strong>Total Party:</strong> {{ count($parties) }} </div>
                    <br>
    ...
    <div class="modal fade" id="edit" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">Edit Party</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
                </div>
                <div class="modal-body">
                    <form id="signupForm" class="editSupplierForm row">
                        ...
                        {{-- <div class="mb-3 col-md-6">
                            ...
                            <input id="defaultconfig" class="form-control edit_opening_receivable" maxlength="39"
                                name="opening_receivable" type="number">
                            <span class="text-danger edit_opening_receivable_error"></span>
                        </div>
                        <div class="mb-3 col-md-6">
                            <label for="name" class="form-label">Opening Payable</label>
                            <input id="defaultconfig" class="form-control edit_opening_payable" maxlength="39"
                                name="opening_payable" type="number">
                            <span class="text-danger edit_opening_payable_error"></span>
                        </div> --}}
                        <div class="mb-3 col-md-12">
                            <label for="name" class="form-label">Party Type</label>
                            <select name="party_type" class="form-control edit_party_type">
                                <option value="both">Both</option>
                                <option value="customer">Customer</option>
                                <option value="supplier">Supplier</option>
                            </select>
                            <span class="text-danger edit_party_type_error"></span>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function getBadgeClass(party_type) {
                party_type = party_type.trim().toLowerCase();
                switch (party_type) {
                    case "supplier":
                        return "bg-primary";
                    case "customer":
                    ...
                    case "both":
                        return "bg-success";
                    default:
                        return "bg-secondary"; // For unknown types
                }
            }
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if ($.fn.DataTable.isDataTable('#partyTable')) {
                            $('#partyTable').DataTable().clear().destroy();
                        }
                        // Check if suppliers data is present
                        if (suppliers.length > 0) {
                            $.each(suppliers, function(index, supplier) {
                                // console.log(supplier.party_type);
                                const isFirstSupplier = supplier.id === $default.id;
                                const tr = document.createElement('tr');
                                tr.innerHTML = `
                                        <td>${index + 1}</td>
                                        <td>
                                        <a href="/party/profile/${supplier.id}" >
                                            ${supplier.name ?? ""}
                                        </a>
                                        </td>
                                        <td>${supplier.phone ?? ""}</td>
                                        <td>${supplier.total_payable ?? ""}</td>
                                        <td>${supplier.total_receivable ?? ""}</td>
                                        <td>${supplier.wallet_balance ?? ""}</td>
                                        <td>
                                            <span class="badge ${getBadgeClass(supplier.party_type)} text-capitalize">${supplier.party_type ?? ""}</span>
                                        </td>
                                        <td>
                                             @can('supplier.edit')
                                            ...
                                            @endcan
                                             @can('supplier.delete')
                                           <a href="#" class="btn btn-danger btn-icon supplier_delete" data-id="${supplier.id}" ${isFirstSupplier ? 'disabled style="pointer-events: none; opacity: 0.5;"' : ''}>
                                    ...
                                    <tr>
                                        <td colspan='8'>
                                            ...
                                            <div class="text-center">
                                                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModalLongScollable">Add Party<i data-feather="plus"></i></button>
                                            </div>
                                            @endcan
                                        </td>
                                    </tr>`);
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status == 200) {
                            ...
                            $('.party_type').val(res.supplier.party_type);
